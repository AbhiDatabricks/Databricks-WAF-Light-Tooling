...
def publish_dashboard(dashboard_id):
    ...
    if response.status_code in [200, 201]:
        ...
        if verify_response.status_code == 200:
            ...
            print(f"   ✅ Verified dashboard exists: {dashboard_name}")
            print(f"   📊 Confirmed Dashboard ID: {verified_id}")
            
            if verified_id != dashboard_id:
                print(f"   ⚠️  WARNING: Dashboard ID mismatch!")
                print(f"      Expected: {dashboard_id}")
                print(f"      Got: {verified_id}")
                return False, warehouse_id
        else:
            print(f"   ⚠️  Could not verify dashboard: {verify_response.status_code}")
...
def update_app_with_embed_url(dashboard_id):
    ...
    try:
        ...
        if "WAF Guide" in app_content or "st.sidebar" in app_content:
            print("✅ Detected comprehensive app.py with WAF Guide")
            ...
            print(f"✅ Updated dashboard ID in comprehensive app.py")
            print(f"   📍 Dashboard ID: {dashboard_id}")
            print(f"   📍 Embed URL: {embed_url}")
        else:
            # Simple version - write basic app
            print("⚠️  Simple app.py detected, writing basic version")
...
'''
        with open(app_path, "w", encoding="utf-8") as f:
            f.write(app_code)
        print(f"✅ Created basic app.py with embed URL")
        print(f"   📍 Embed URL: {embed_url}")
